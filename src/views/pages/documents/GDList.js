...
export default function GDList() {
    ...
    const getList = () => {
        ...
        AxiosServices.getGDList({ ...filter, pageNumber: page, pageSize: rowsPerPage }).then((res) => {
    ...
    const downLoadAllDocument = () => {
        ...
        handleLoaderDispatch(true, "Please wait while we are downloading you file.", docuemntImage)
    ...
    const columnConfig = [
        { name: 'File No', col: 'fileNo', pdf: false, hasChip: false },
        { name: 'Consignee', col: 'consignee', pdf: false, hasChip: false },
        { name: 'Bill of Lading', col: 'billOfLading', pdf: false, hasChip: false },
        { name: 'Date', col: 'creationDate', pdf: false, hasChip: false , width: 100},
        { name: 'Status', col: 'rfpStatus', pdf: false, hasChip: true },
        { name: 'GD', col: 'GD', pdf: true, hasChip: false },
        ...
        return list.map((row, rowIndex) => {
            ...
            return (
                <TableRow key={rowIndex} hover>
                    ...
                    {columnConfig.map((column, colIndex) => (
                        column.hasChip ?
                            <TableCell key={colIndex} align="left">
                                <Chip size="small" label={row[column.col]} style={{ color: getChipColor(row[column.col]), borderColor: getChipColor(row[column.col]) }} variant="outlined" />
                            </TableCell> :
                            column.pdf ?
                                <TableCell key={colIndex} align="left">
                                    <DownloadFile
                    ...
                    {/* <TableCell sx={{ minWidth: 220 }} align="center">
    ...
    return (
        <Grid container spacing={2}>
            ...
            {open && <ViewDetails isLoading={isProcessing} open={open} handleClose={handleClose} item={item} onApproval={onApproval} />}
        </Grid>
    );
}
