...
import ReimbursementCategoryDialog from "./reimbursementCategoryDialog";
...
export default function ReimbursementCategoryList() {
    ...
    const handleStatus = (row) => {
        ...
        let model = {
            flag: !row.isActive,
            reimbursmentCategoryID: row.reimbursmentCategoryID
        }

        AxiosServices.deActivatedReimbursementCategory([model]).then((res) => {
    ...
    const getList = () => {
        ...
        AxiosServices.getReimbursmentCategoryList(model).then((res) => {
    ...
    const columnConfig = [
        { name: 'Reimbursement Category', col: 'reimbursmentCategoryName' },
    ...
    return (
        <Grid container spacing={2}>
            <Grid item xs={12}>
                ...
                >Add Reimbursement Category</Button>
            </Grid>
            <Grid item xs={12}>
                <MainCard
                    content={false}
                    title="Reimbursement Category List"
                    secondary={null}
                >
                    {/* table */}
                    <CustomTable
            ...
            {open && <ReimbursementCategoryDialog open={open} handleClose={handleClose} item={item} getList={getList} />}
        </Grid>
    );
}
